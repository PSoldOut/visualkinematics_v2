...
class Inspector:
    ...
    def _create_theta_sliders(self):
        ...
        for j in self.manipulator.joints:
            ...
            name_and_slider_and_sign = self._create_theta_slider(num, value=0, joint=j, callback=self._on_theta_slider)
    ...
    def _create_theta_slider(self, num, value, joint, callback: Callable[[], None] = None):
        ...
        return [joint.name, theta_rot_slider, sign]
...
class Environment:
    ...
    def add_inspector(self, obj):
        ...
        self.add_widget(inspector.widget)
        return inspector
    ...
    class Gizmo_Controls:
        ...
        def __init__(
            ...
            if scale:
                ...
                def _on_scale_slider(change):
                    util.set_scale(self.obj_renderable, [self.x_scale_slider.value, self.y_scale_slider.value, self.z_scale_slider.value])
                    if callback is not None : callback()
        ...
        def _on_rot_slider(self, change):
            ...
            if (o == "zyz" or o == "ZYZ" or
                ...
                o == "zxz" or o == "ZXZ"):
                if self.local_space_check_box.value:
                    util.set_rotation(self.obj_renderable, [self.x_rot_slider.value, self.y_rot_slider.value, self.z_rot_slider.value], self.rotation_order_dropdown.value)
                else:
                    util.set_rotation_global(self.obj_renderable, [self.x_rot_slider.value, self.y_rot_slider.value, self.z_rot_slider.value], self.rotation_order_dropdown.value)
            else:
                angles = util.order_angles([self.x_rot_slider.value, self.y_rot_slider.value, self.z_rot_slider.value], "XYZ", self.rotation_order_dropdown.value)
                if self.local_space_check_box.value:
                    util.set_rotation(self.obj_renderable, angles, self.rotation_order_dropdown.value)
                else:
                    util.set_rotation_global(self.obj_renderable, angles, self.rotation_order_dropdown.value)
            if self.callback is not None : self.callback()
        ...
        def _on_local_space_check_box(self, change):
            ...
            self.z_rot_slider.unobserve(self._on_rot_slider, names="value")
            if self.local_space_check_box.value:
                euler = util.quaternion_to_euler(self.obj.quaternion[0], self.obj.quaternion[1], self.obj.quaternion[2], self.obj.quaternion[3], self.rotation_order_dropdown.value)
                euler = util.order_angles(euler, self.rotation_order_dropdown.value, "XYZ")
                self.x_rot_slider.value = euler[0]
                self.y_rot_slider.value = euler[1]
                self.z_rot_slider.value = euler[2]
            else:
                euler = util.quaternion_to_euler(self.obj.quaternion[0], self.obj.quaternion[1], self.obj.quaternion[2], self.obj.quaternion[3], self.rotation_order_dropdown.value[::-1])
                euler = util.order_angles(euler, self.rotation_order_dropdown.value[::-1], "XYZ")
                self.x_rot_slider.value = euler[0]
                self.y_rot_slider.value = euler[1]
                self.z_rot_slider.value = euler[2]
            self.x_rot_slider.observe(self._on_rot_slider, names="value")
